...
namespace RosSharp.RosBridgeClient
{
    public class TwistPublisher : UnityPublisher<MessageTypes.Geometry.Twist>
    {
        ...
        private void UpdateMessage()
        {
            ...
            Vector2 coord2 = OVRInput.Get(OVRInput.Axis2D.SecondaryThumbstick);
            ...
            message.linear.x = coord.y*6;
            message.linear.y = -coord.x*6;
            message.angular.z = -coord2.x * 18;

            if (coord2.x > 0)
            {
                message.linear.x = message.linear.x + 0.01;
            }
            else if (coord2.x < 0)
            {
                message.linear.x = message.linear.x + (-0.01);
